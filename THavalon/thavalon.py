...
def get_player_info(player_names, gameroom, roles):
	...
	if(len(roles) > 0):
		...
		evil_roles.append('Agravaine')
		evil_roles.append('Colgrevance')
		good_roles = set(good_roles) & set(roles)
		evil_roles = set(evil_roles) & set(roles)

		team_count = { 5 : { "good" : 3, "evil" : 2 },
			6 : { "good" : 4, "evil" : 2},
			7 : { "good" : 4, "evil" : 3},
			8 : { "good" : 5, "evil" : 3},
			9 : { "good" : 6, "evil" : 3},
			10 : { "good" : 6, "evil" : 4}}
		if(len(good_roles) < team_count.get(num_players).get("good")):
			return "Not enough good roles selected"
		if(len(evil_roles) < team_count.get(num_players).get("evil")):
			return "Not enough evil roles selected"
	...
	if os.path.isdir("game"+str(gameroom)):
		shutil.rmtree("game"+str(gameroom))
	os.mkdir("game"+str(gameroom))

	bar = '----------------------------------------\n'
	for player in players:
		player.string = bar+'You are '+player.role+' ['+player.team+']\n'+bar+get_role_description(player.role)+'\n'+bar+'\n'.join(player.info)+'\n'+bar
		player_file = "game" + str(gameroom) + "/{}".format(player.name)
		with open(player_file,"w") as file: 
			file.write(player.string)

	first_player = random.sample(players,1)[0]
	with open("game" + str(gameroom) + "/start", "w") as file:
	...
	with open("game" + str(gameroom) + "/DoNotOpen", "w") as file:
...
def run_THavalon( players, gameroom_array, roles):
	...
	if len(players) != num_players:
		...
		return "No duplicate player names"

	if not (len(gameroom_array) == 1):
		print("Invalid gameroom count")
		return "Invalid gameroom count"

	return get_player_info(players, gameroom_array[0], roles)
